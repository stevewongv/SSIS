...
def masks_copy_paste(masks, paste_masks, alpha):
    if alpha is not None:
        ...
        half_len = int(len(masks)/2)
        half_len_p = int(len(paste_masks)/2)
        new_masks.extend(masks[half_len:])
        new_masks.extend(paste_masks[half_len_p:])
        new_masks.extend(masks[:half_len])
        new_masks.extend(paste_masks[:half_len_p])
        masks = new_masks
...
def bboxes_copy_paste(bboxes, paste_bboxes, masks, paste_masks, alpha, key):
    if key == 'paste_bboxes':
        ...
        half_len = int(len(adjusted_bboxes)/2)
        half_len_p = int(len(adjusted_paste_bboxes)/2)
        bboxes = adjusted_bboxes[:half_len] + adjusted_paste_bboxes[:half_len_p] + adjusted_bboxes[half_len:] + adjusted_paste_bboxes[half_len_p:]
...
class CopyPaste(A.DualTransform):
    ...
    def get_params_dependent_on_targets(self, params):
        ...
        #select objects
        objs_to_paste = range(0, n_objects)
